# F1 Points Engine — Roadmap

## Phase 1: Core Fantasy Tool ✅ *Complete*

Everything a fantasy player needs to compete seriously.

**Team Builder + Optimizer**
- Manual team picker with driver/constructor cards (price, form, value score)
- PuLP optimizer: Max Points team + Best Value team
- Real-time budget tracker
- DRS Boost selector

**Fantasy Scoring Engine**
- Full rules implementation: qualifying, sprint, grand prix
- All bonuses: positions gained/lost, overtakes, fastest lap, pit stops
- Chip effects: DRS Boost (2×/3×), No Negative, Wildcard, Limitless, Autopilot, Final Fix
- Score validation layer: cross-check our computed scores vs official F1 Fantasy API after each race

**Live Race Tracker**
- OpenF1 API integration (lap-by-lap position, pit stops, fastest lap)
- WebSocket push to frontend every 30 seconds during sessions
- Session-aware: auto-applies qualifying vs sprint vs race scoring rules
- Live points table with delta indicators

**Chip Advisor**
- Rule-based recommendations (street circuit → No Negative, banked transfers → Wildcard, etc.)
- Confidence rating + plain-English explanation

**Standings Page**
- F1 Drivers Championship (WDC) with points progression chart
- F1 Constructors Championship (WCC)
- Fantasy value leaderboard (points per $M)
- `GET /api/standings/progression?season=` — cumulative fantasy points per driver per round, computed from DB (no Ergast dependency); chart shows all 24 real rounds

**Expected Points (xP)**
- Rolling weighted average with circuit-type multiplier
- Shown on all driver cards, used by optimizer

**Data Integrity (resolved bug fixes)**
- Arvid Lindblad (not Ayumu) — code LIN, Swedish, Racing Bulls
- 2026 team assignments: Colapinto→Alpine, Perez/Bottas→Cadillac, Hulkenberg/Bortoleto→Audi
- 2025 mid-season swap: Lawson at Red Bull rounds 1–2, Tsunoda at Red Bull rounds 3–24
- TSU + DOO seeded as 2025-only drivers (price=0) — 24 rounds of race results preserved
- Dashboard: Races Remaining shows full season count; days-until-next-race subtitle; "Season Starts" label pre-season
- Team Builder: content scrolls above sticky mobile footer (`pb-44 sm:pb-0`)

---

## Phase 2: Intelligence Layer ✅ *Complete*

Turns good fantasy players into great ones. FPL-inspired analytical depth.

**Form vs Luck Detector** ✅
- `GET /api/drivers/{id}/form` — last 5 races actual vs xP, delta, flag
- 🔴 Sell signal (overperforming) / 🟢 Buy signal (underperforming) badges on DriverCard
- Recharts AreaChart sparkline (lazy-fetched on card expand)

**Circuit Intelligence** ✅
- `DriverCircuitProfile` DB table — avg_points per driver per circuit type (street/power/balanced)
- `GET /api/drivers/circuit-fit?circuit_type=` — ranked fit scores (0–10)
- `GET /api/races/upcoming-difficulty?drivers=` — next 5 races with driver fit tiles
- Circuit fit badge on DriverCard; "Fixture View" tab on Standings page (colour-coded green/amber/red)

**Differential Finder** ✅
- `is_differential` flag: top-30% xP AND price < $12M
- ⚡ badge on DriverCard
- "Show Differentials Only" toggle in TeamBuilder

**Teammate Comparison** ✅
- `GET /api/constructors/{id}/teammates` + `GET /api/drivers/{id}/vs-teammate`
- "Compare Teammates" button on ConstructorCard
- TeammateModal: bottom-sheet (mobile) / centered (desktop) with stat table + Recharts BarChart

**Transfer Planner** ✅
- `GET /api/transfers/plan?drivers=&constructors=` — 3-race plan timeline
- Drop/add suggestions with budget delta; chip alternative flag (amber)
- Collapsible "Transfer Planner" section in TeamBuilder (lazy-fetched)

---

## Phase 3: Championship Simulator 🔮 *The non-fantasy hook*

Makes the tool useful for every F1 fan, not just fantasy players. This is the distribution and marketing engine.

**Interactive Title Odds Calculator**
- Select your favourite driver
- See their current championship win probability (computed via Monte Carlo simulation)
- Simulation runs remaining races thousands of times, weighted by each driver's historical finish distribution
- Updates live after every race

**Scenario Builder**
- Drag-and-drop finishing positions for upcoming races
- "What if Max retires from the next 2 races?" → see probability shift instantly
- "What does George need to do in the last 5 races to win?" → system computes minimum required results
- Shareable scenarios via URL (e.g. `?scenario=VER:DNF,NOR:P1,PIA:P3`)

**Championship Math Panel**
- "Driver X is mathematically eliminated from the title" alerts
- Maximum possible points remaining per driver
- Points gap to leader with races remaining

**Season Replay**
- Replay any historical F1 season through the simulator
- "What if Senna hadn't crashed at Suzuka 1989?" style scenarios
- Uses Ergast historical data back to 2003

---

## Mobile-First UI ✅ *Complete*

The app is optimised for use during live race weekends on a phone:

- Bottom navigation bar on mobile (5 tabs: Home, Team, Live, Standings, Chips)
- Team Builder: single-column cards, sticky footer with DRS pill row + budget bar + full-width Optimize button
- Live Race Tracker: sticky session progress bar, tap-to-expand rows (code + pos + pts → full breakdown), accessible ▲/▼ delta indicators
- Standings: horizontally scrollable championship chart, sticky first column on tables, top-5 default with "Show all" toggle
- Chip Advisor: full-width recommendation card, prominent confidence badge, 52px action button
- All tap targets ≥44px, minimum 14px body font, `overflow-x-hidden` root

---

## Future Ideas (Unscheduled)

- **ML chip advisor**: replace rules with a model trained on historical chip outcomes
- **Multi-user leagues**: leaderboard for groups of friends using the tool together
- **Push notifications**: "Race starts in 1 hour — set your DRS Boost" reminder
- **Price prediction model**: predict next week's price changes using xP vs current price
- **PWA offline support**: service worker for race weekend use without connectivity

---

*Roadmap version: 1.3 | Last updated: March 2026*
